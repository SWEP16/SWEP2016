...
namespace commands {
    namespace simplecommands {
        class ExportToExcelCommand : SimpleCommand {
            MeasurementSeriesCollection m;

            public ExportToExcelCommand(MeasurementSeriesCollection msc) {
                this.m = msc;
            }

            public void execute() {
                this.createExcel(this.m);
            }
                    ...
                    for(int i = 0; i < msc.getMeasurementSeriesLength(); i++) {
                        ...
                        //Way Time Measurement Series
                            ...
                            for(int j = 0; j < wts.getMeasurementsLength(); j++) {
                                newSheet.Cells[row, col + 0] = wts.getMeasurement(j).Item1;
                                newSheet.Cells[row, col + 1] = wts.getMeasurement(j).Item2;
                                row++;
                            }
                            row = 1;
                            col = 1;
